...
const MENU = [
  ...
  { href: "/cost-details", label: "Cost Details", icon: CostIcon },
...
export function Sidebar({ onNewTransaction }: { onNewTransaction: () => void }) {
  ...
  return (
    <aside className={`flex h-screen flex-col border-r border-border bg-card transition-all ${collapsed ? "w-16" : "w-64"}`}>
        ...
        {items.map((item) => {
          ...
          return (
            <li key={item.href}>
              <Link
                href={item.href}
                className={`flex items-center gap-3 rounded-control px-2.5 py-2 text-sm transition-colors ${
